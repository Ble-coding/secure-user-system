...
const handleDeleteClick = (
  index: number,
  child: FieldArrayWithId<ParentFormData, "children", "id">
) => {
  const childName = `${child.first_name || ""} ${child.last_name || ""}`.trim() || `Enfant ${index + 1}`

  // ✅ VRAIE condition : si enfant existant en base (dbId présent)
  const hasData = typeof child.dbId === "number"

  setDeleteModal({ isOpen: true, index, childName, hasData })
}
              ...
              <div className="flex justify-between items-center">
                ...
                {!isReadOnly && fields.length > 1 && (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => handleDeleteClick(index, field)}
                  >
